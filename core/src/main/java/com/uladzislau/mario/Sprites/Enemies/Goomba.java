...
public class Goomba extends Enemy{
    ...
    public Goomba(PlayScreen screen, float x, float y) {
        ...
        for(int i = 0; i < 2; i++){
            frames.add(new TextureRegion(screen.getAtlas().findRegion("Mario GFX/goomba"), i*16, 0, 16, 16));
        }
    ...
    public void update(float dt){
        ...
        if(setToDestroy && !destroyed){
            ...
            destroyed = true;
            setRegion(new TextureRegion(screen.getAtlas().findRegion("Mario GFX/goomba"), 32, 0, 16, 16));
            stateTime = 0;
        }
        else if(!destroyed) {
    ...
    public void hitOnHead(MarioSprite mario) {
        setToDestroy = true;
        Mario.assetManager.get("audio/sounds/stomp.wav", Sound.class).play();
